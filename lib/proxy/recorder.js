...
const {sendFront} = require('../../web/ws');

const BODY_FILE_PRFIX = 'res_body_';
const CACHE_DIR_PREFIX = 'cache_r';
function getCacheDir() {
  const rand = Math.floor(Math.random() * 1000000),
    cachePath = path.join(proxyUtil.getAnyProxyPath('cache'), './' + CACHE_DIR_PREFIX);
...
function normalizeInfo(id, info) {
  ...
  if (info.endTime) {
    ...
    if (contentType) {
      ...
      singleRecord.mime = singleRecord.mime.split('/').pop();
    } else {
      singleRecord.mime = '';
    }
...
class Recorder extends events.EventEmitter {
  ...
  updateRecord(id, info, flag) {
    ...
    //发送到客户端
    flag && sendFront(Object.assign({}, {info: finalInfo}, {type: 'recorder'}));
  }
  ...
  clear() {
    const self = this;
    proxyUtil.deleteFolderContentsRecursive(self.cachePath, false);
  }
}
